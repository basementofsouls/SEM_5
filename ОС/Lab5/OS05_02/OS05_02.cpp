...
BOOL CreateProcess(
  LPCTSTR lpApplicationName,        // Имя исполняемого файла или NULL
  LPTSTR lpCommandLine,             // Командная строка для запуска
  LPSECURITY_ATTRIBUTES lpProcessAttributes, // Необязательные атрибуты безопасности для процесса
  LPSECURITY_ATTRIBUTES lpThreadAttributes,  // Необязательные атрибуты безопасности для потока
  BOOL bInheritHandles,             // Указывает, могут ли дочерние процессы наследовать дескрипторы текущего процесса
  DWORD dwCreationFlags,            // Флаги создания процесса
  LPVOID lpEnvironment,             // Указатель на окружение (необязательно)
  LPCTSTR lpCurrentDirectory,       // Текущий рабочий каталог для процесса (необязательно)
  LPSTARTUPINFO lpStartupInfo,      // Структура STARTUPINFO, указывает параметры создания процесса
  LPPROCESS_INFORMATION lpProcessInformation // Структура PROCESS_INFORMATION, в которой будет сохранена информация о процессе
);
