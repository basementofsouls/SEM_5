...
// j - номер столбца
{
	...
	{
		TCHAR* error = (TCHAR*)_T("CMatrix::operator(int,int): выход индекса за границу диапазона ");
...
// i - номер строки для вектора
{
	...
	{
		char* error = (char*)"CMatrix::operator(int): объект не вектор - число столбцов больше 1 ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	if (i > n_rows - 1)     //  проверка выхода за диапазон
	{
		TCHAR* error = (TCHAR*)TEXT("CMatrix::operator(int): выход индекса за границу диапазона ");
...
CMatrix CMatrix::operator-()// Оператор -M
{
...
CMatrix CMatrix::operator+(CMatrix & M)// Оператор M1+M2
{
	int bb = (n_rows == M.rows()) && (n_cols == M.cols());
	if (!bb)
	{
		char* error = (char*)"CMatrix::operator(+): несоответствие размерностей матриц ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix Temp(*this);
...
CMatrix CMatrix::operator*(CMatrix & M)// Умножение на матрицу M
{
	double sum;
	int nn = M.rows();
	int mm = M.cols();
	...
	{
		TCHAR* error = (TCHAR*)TEXT("CMatrix::operator*: несоответствие размерностей матриц ");
		MessageBox(NULL, error, TEXT("Ошибка"), MB_ICONSTOP);
		exit(1);
	}
	return Temp;
...
CMatrix CMatrix::operator=(const CMatrix & M)// Оператор присваивания M1=M
{
...
CMatrix CMatrix::operator+(double x)// Оператор M+x, где M - матрица, x - число
{
	CMatrix Temp(*this);
	for (int i = 0; i < n_rows; i++)
...
CMatrix CMatrix::Transp()// Возвращает матрицу,транспонированную к (*this)
{
...
CMatrix CMatrix::GetRow(int k)// Возвращает строку матрицы по номеру k
{
	if (k > n_rows - 1)
	{
		char* error = (char*)"CMatrix::GetRow(int k): параметр k превышает число строк ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix M(1, n_cols);
...
// m - номер последнего элемента в строке
{
	...
	if (!b4)
	{
		char* error = (char*)"CMatrix::GetRow(int k,int n, int m):ошибка в параметрах ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	int nCols = m - n + 1;
...
CMatrix CMatrix::GetCol(int k)// Возвращает столбец матрицы по номеру k
{
	if (k > n_cols - 1)
	{
		char* error = (char*)"CMatrix::GetCol(int k): параметр k превышает число столбцов ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix M(n_rows, 1);
...
// m - номер последнего элемента в столбце
{
	...
	if (!b4)
	{
		char* error = (char*)"CMatrix::GetCol(int k,int n, int m):ошибка в параметрах ";
		MessageBox(NULL, (LPCWSTR)error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	int nRows = m - n + 1;
...
double CMatrix::MaxElement()//// Максимальное значение элементов матрицы
{
	double max = (*this)(0, 0);
...
double CMatrix::MinElement()// Минимальное значение элементов матрицы
{
	double min = (*this)(0, 0);
	for (int i = 0; i < (this->rows()); i++)
...
void PrintMatrix(CDC& dc, int x, int y, CMatrix& M)		
{
	...
	for (int i = 0; i < M.rows(); i++)
	{
		for (int j = 0; j < M.cols(); j++)
		{
			...
			str2.erase(str2.length()-4,4);
			str1 = (char*)str2.c_str();
			str += str1;
			str += _T(' ');
		}
		dc.TextOutW(x, y + i * 25, str);
		str = _T("");
	}
}
